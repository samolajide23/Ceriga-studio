    ...
    <>
      {!open && (
        <button
          ...
          className={cn(
            "fixed z-[95] flex items-center gap-2.5 rounded-2xl border border-white/12 bg-ceriga-elevated px-3 py-2.5 text-white shadow-[0_12px_40px_rgba(0,0,0,0.45)] transition-transform hover:border-white/18 hover:bg-[#161618] active:scale-[0.98] sm:px-3.5",
          ...
          aria-expanded={open}
        >
          <span className="relative flex h-9 w-9 shrink-0 items-center justify-center rounded-xl border border-ceriga-accent/30 bg-ceriga-accent/12">
            <MessageCircle className="h-5 w-5 text-[#FCA5A5]" strokeWidth={2} aria-hidden />
            <span className="absolute -bottom-0.5 -right-0.5 flex h-4 w-4 items-center justify-center rounded-full border border-[#2d2d2d] bg-ceriga-accent">
      ...
      <Sheet open={open} onOpenChange={setOpen}>
        <SheetContent
          ...
          side="right"
          className={cn(
            "flex flex-col gap-0 overflow-hidden bg-ceriga-elevated p-0 text-white [&>button:last-child]:hidden",
