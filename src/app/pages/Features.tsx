    ...
    <MarketingShell>
      ...
      <section className="px-[max(1rem,env(safe-area-inset-left))] py-10 pr-[max(1rem,env(safe-area-inset-right))] sm:px-6 sm:py-14 md:px-8 lg:px-10">
        <div className="mx-auto grid min-w-0 max-w-[1100px] grid-cols-1 items-center gap-5 min-[560px]:grid-cols-2 min-[560px]:gap-4 md:gap-8 lg:gap-12">
          <div className="relative mx-auto min-w-0 w-full max-w-[280px] overflow-hidden rounded-[14px] border border-white/[0.08] bg-ceriga-bg min-[560px]:mx-0 min-[560px]:max-w-none">
            ...
            <div className="pointer-events-none absolute inset-0 rounded-[inherit] bg-gradient-to-t from-ceriga-bg via-transparent to-transparent" />
            <p className="absolute bottom-2 left-2 right-2 text-[8px] font-semibold uppercase  text-white/40 sm:bottom-3 sm:left-3 sm:text-[9px] md:text-[10px]">
              Visual builder · Flat-lay sync
            </p>
          </div>
          <div className="min-w-0">
            <p className="text-[12px] font-medium text-ceriga-accent text-ceriga-accent">Core workflow</p>
            <h2 className="mt-1.5 font-display text-lg font-bold tracking-tight text-ceriga-text sm:text-xl md:text-2xl">
            ...
            <ul className="mt-5 space-y-3 sm:mt-6 sm:space-y-4 md:mt-8 md:space-y-5">
              {spotlight.map(({ icon: Icon, title, body }) => (
                <li key={title} className="flex gap-3 sm:gap-4">
                  <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg border border-white/10 bg-black/40 text-ceriga-accent sm:h-9 sm:w-9 sm:rounded-xl md:h-10 md:w-10">
                    <Icon className="h-4 w-4 sm:h-[18px] sm:w-[18px] md:h-5 md:w-5" />
                  </div>
                  <div className="min-w-0">
                    <h3 className="text-sm font-semibold text-ceriga-text sm:text-[15px]">{title}</h3>
      ...
      <section className="border-t border-white/[0.06] bg-ceriga-bg px-[max(1rem,env(safe-area-inset-left))] py-12 pr-[max(1rem,env(safe-area-inset-right))] sm:px-6 sm:py-16 md:px-8 lg:px-10">
        <div className="mx-auto max-w-[1100px]">
          <p className="text-[12px] font-medium text-ceriga-accent text-ceriga-accent">Also included</p>
          <h2 className="mt-2 font-display text-lg font-bold text-ceriga-text sm:text-xl">
          ...
          <div className="mt-8 grid gap-4 sm:grid-cols-2">
            {moreFeatures.map(({ icon: Icon, title, body }) => (
              <article
                key={title}
                className="rounded-[14px] border border-white/[0.08] bg-ceriga-elevated p-5 sm:p-6"
              >
                <div className="mb-3 flex h-9 w-9 items-center justify-center rounded-lg border border-white/10 bg-black/40 text-ceriga-accent">
                  <Icon className="h-4 w-4" />
                </div>
                <h3 className="text-sm font-semibold text-ceriga-text">{title}</h3>
      ...
      <section className="px-[max(1rem,env(safe-area-inset-left))] py-12 pr-[max(1rem,env(safe-area-inset-right))] sm:px-6 sm:py-16 md:px-8 lg:px-10">
        <div className="mx-auto grid min-w-0 max-w-[1100px] grid-cols-1 items-center gap-5 min-[560px]:grid-cols-2 min-[560px]:gap-4 md:gap-10 lg:gap-14">
          <div className="order-2 min-w-0 min-[560px]:order-1">
            <p className="text-[12px] font-medium text-ceriga-accent text-ceriga-accent">Quality bar</p>
            <h2 className="mt-1.5 font-display text-lg font-bold tracking-tight text-ceriga-text sm:text-xl md:text-2xl">
            ...
            <ul className="mt-5 max-w-md space-y-2 text-xs text-white/50 sm:mt-6 sm:space-y-2.5 sm:text-sm md:mt-8 md:space-y-3">
              {[
                ...
                <li key={line} className="flex gap-2 sm:gap-3">
                  <Check className="mt-0.5 h-3.5 w-3.5 shrink-0 text-ceriga-accent sm:h-4 sm:w-4" />
                  <span>{line}</span>
                </li>
              ))}
            </ul>
            <div className="mt-6 flex flex-col gap-2.5 sm:mt-8 sm:flex-row sm:gap-3 md:mt-10 md:gap-4">
              <Button asChild className="h-9 bg-ceriga-accent px-5 text-xs font-semibold hover:bg-ceriga-accent-hover/90 sm:h-10 sm:px-6">
          ...
          <div className="order-1 mx-auto min-w-0 w-full max-w-[280px] overflow-hidden rounded-[14px] border border-white/[0.08] bg-ceriga-elevated min-[560px]:order-2 min-[560px]:mx-0 min-[560px]:max-w-none">
